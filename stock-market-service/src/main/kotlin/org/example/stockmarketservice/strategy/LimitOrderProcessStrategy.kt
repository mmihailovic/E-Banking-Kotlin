package org.example.stockmarketservice.strategy

import com.google.gson.Gson
import org.example.stockmarketservice.exceptions.StockDoesntExistException
import org.example.stockmarketservice.model.Stock
import org.example.stockmarketservice.model.order.LimitOrder
import org.example.stockmarketservice.model.order.Order
import org.example.stockmarketservice.model.order.OrderAction
import org.example.stockmarketservice.repository.ListingOwnerRepository
import org.example.stockmarketservice.repository.OrderRepository
import org.example.stockmarketservice.repository.StockRepository
import org.springframework.amqp.rabbit.core.RabbitTemplate
import org.springframework.beans.factory.annotation.Value
import org.springframework.stereotype.Component

@Component("limitOrder")
class LimitOrderProcessStrategy(
    @Value("\${update.balance.exchange}") UPDATE_BALANCE_EXCHANGE: String,
    @Value("\${update.balance.routing.key}") UPDATE_BALANCE_ROUTING_KEY: String,
    rabbitTemplate: RabbitTemplate,
    gson: Gson,
    orderRepository: OrderRepository,
    private val stockRepository: StockRepository,
    listingOwnerRepository: ListingOwnerRepository,
) : AbstractOrderProcessStrategy(
    UPDATE_BALANCE_EXCHANGE, UPDATE_BALANCE_ROUTING_KEY, orderRepository, listingOwnerRepository, rabbitTemplate, gson
) {
    override fun processOrder(order: Order) {
        if (!isOrderExecutable(order)) {
            return
        }

        val ticker: String = order.ticker

        val stock: Stock = stockRepository.findById(ticker)
            .orElseThrow { StockDoesntExistException(ticker) }

        if (order.orderAction == OrderAction.BUY) {
            processBuyOrder(order, stock.high, stock.low)
        } else {
            processSellOrder(order, stock.high, stock.low)
        }
    }

    public override fun isOrderExecutable(order: Order): Boolean {
        val ticker: String = order.ticker
        val stock: Stock = stockRepository.findById(ticker).orElseThrow { StockDoesntExistException(ticker) }

        val limitOrder: LimitOrder = order as LimitOrder

        return (stock.low > limitOrder.limitAmount && limitOrder.orderAction == OrderAction.SELL) ||
                (stock.high < limitOrder.limitAmount && limitOrder.orderAction === OrderAction.BUY)
    }
}